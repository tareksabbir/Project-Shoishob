...
const LeaderBoard = () => {
  ...
  const getRankStyle = (position) => {
    if (position === 0) return "bg-gradient-to-r from-yellow-900/20 to-yellow-800/20 border-l-4 border-yellow-400";
    if (position === 1) return "bg-gradient-to-r from-gray-900/20 to-gray-800/20 border-l-4 border-gray-300";
    if (position === 2) return "bg-gradient-to-r from-amber-900/20 to-amber-800/20 border-l-4 border-amber-600";
    if (position < 5) return "bg-gradient-to-r from-purple-900/20 to-purple-800/20 border-l-4 border-purple-400";
            ...
            return (
              <div
                ...
                className={`relative bg-slate-800/60 backdrop-blur-sm rounded-2xl p-6 border border-slate-700/50 transform transition-all duration-300 hover:scale-105 hover:bg-slate-700/60 ${
                  actualIndex === 0 ? 'md:order-2 ring-2 ring-yellow-400/30' :
                  actualIndex === 1 ? 'md:order-1' : 'md:order-3'
                }`}
              >
                <div className="text-center">
                  <div className={`w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-br ${
                    actualIndex === 0 ? 'from-yellow-400 to-yellow-600' :
                    actualIndex === 1 ? 'from-gray-300 to-gray-500' :
                  ...
                  <h3 className="text-white font-semibold text-lg mb-1">{podiumUser?.name}</h3>
                  <p className="text-slate-400 text-sm mb-2">Chittagong</p>
                  <div className="bg-slate-700/50 rounded-lg p-3">
                    <p className="text-2xl font-bold text-white">{podiumUser?.point}</p>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gradient-to-r from-purple-900/50 to-pink-900/50 border-b border-slate-700/50">
                  ...
                  <th className="text-left py-4 px-6 text-slate-300 font-semibold">Location</th>
                  <th className="text-left py-4 px-6 text-slate-300 font-semibold">Matches</th>
                  <th className="text-left py-4 px-6 text-slate-300 font-semibold">Points</th>
                  <th className="text-left py-4 px-6 text-slate-300 font-semibold">Improvement</th>
                  <th className="text-left py-4 px-6 text-slate-300 font-semibold">Badge</th>
                </tr>
              </thead>
              <tbody>
                {sortedUsers.map((user, index) => {
                  const improvement = user.previousPoint !== undefined 
                    ? ((user.point - user.previousPoint) / user.previousPoint) * 100 
                    : 0;

                  return (
                    <tr
                      key={user._id}
                      className={`transition-all duration-300 hover:bg-slate-700/30 ${getRankStyle(index)}`}
                    >
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center space-x-2">
                          <span className="text-2xl font-bold text-white">{user.point}</span>
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className={`font-semibold ${getImprovementColor(improvement)}`}>
          ...
          <div className="bg-slate-800/60 backdrop-blur-sm rounded-xl p-6 border border-slate-700/50">
            ...
            <p className="text-2xl font-bold text-white">
              {sortedUsers.length > 0 
                ? Math.round(sortedUsers.reduce((sum, user) => sum + user.point, 0) / sortedUsers.length)
                : 0
              }
            </p>
          </div>
